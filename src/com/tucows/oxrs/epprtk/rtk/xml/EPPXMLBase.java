...
public abstract class EPPXMLBase extends RTKBase {
	 ...
	 * 
	 * @see The EPP IDL epp.idl for the definition of the epp_TransferStatusType
	 *      enum
	 */
	...
	public static void initTransferStatusToTypeHash() {
		if (transfer_status_to_type_hash_ == null) {
			...
			transfer_status_to_type_hash_.put("clientApproved",
					epp_TransferStatusType.CLIENT_APPROVED);
			transfer_status_to_type_hash_.put("clientCancelled",
					epp_TransferStatusType.CLIENT_CANCELLED);
			transfer_status_to_type_hash_.put("clientRejected",
					epp_TransferStatusType.CLIENT_REJECTED);
			transfer_status_to_type_hash_.put("pending",
					epp_TransferStatusType.PENDING);
			transfer_status_to_type_hash_.put("serverApproved",
					epp_TransferStatusType.SERVER_APPROVED);
			transfer_status_to_type_hash_.put("serverCancelled",
					epp_TransferStatusType.SERVER_CANCELLED);
		}
	}
	 ...
	 *             if the node list does not contain the result text
	 */
	protected epp_Response parseGenericResult(Node response_node)
		...
		NodeList result_nodes = ((Element) response_node)
		...
		if (result_nodes.getLength() > 0) {
			for (int count1 = 0; count1 < result_nodes.getLength(); count1++) {
				...
				result.m_code = Short.parseShort(an_element
						.getAttribute("code"));
				...
				if (value_nodes.getLength() > 0) {
					for (int count2 = 0; count2 < value_nodes.getLength(); count2++) {
						...
						// getElementsByTagName() is recursive to the children,
						// so we might have
						// a <value> node which is really under an <extValue>
						// parent.
						if (value_node.getParentNode().getNodeName()
								.equals("extValue")) {
							// skip... leave it to when we process <extValue>'s
							// in the next block
							break;
						}
						...
						debug(DEBUG_LEVEL_TWO, method_name,
								"the value contents node name ["
										+ value_contents.getNodeName() + "]");
						...
						try {
							...
							udoc.appendChild(udoc.importNode(value_contents,
									true));
						...
						epp_ResultValue value = new epp_ResultValue(
								value_contents.getNamespaceURI(),
								value_xml_string,
								(good_xml ? value_contents.getNodeName() : null),
								(good_xml
										&& value_contents.getFirstChild() != null ? value_contents
										.getFirstChild().getNodeValue() : null));
						// XXX have to parse stuff here if we can.
						values.add(value);
					}
					result.m_values = (epp_ResultValue[]) convertListToArray(
							(new epp_ResultValue()).getClass(), values);

				}

				NodeList ext_value_nodes = an_element
						.getElementsByTagName("extValue");
				...
				if (ext_value_nodes.getLength() > 0) {
					for (int count2 = 0; count2 < ext_value_nodes.getLength(); count2++) {
						...
						NodeList ext_value_children = ext_value_node
								.getChildNodes();

						epp_ExtResultValue ext_value = null;

						for (int count3 = 0; count3 < ext_value_children
								.getLength(); count3++) {
							...
							if (a_node.getNodeName().equals("value")) {
								...
								try {
									Document udoc = new DocumentImpl();
									udoc.appendChild(udoc.importNode(
											value_contents, true));
									value_xml_string = createXMLSnippetFromDoc(udoc);
								} catch (IOException ioe) {
									good_xml = false;
									value_xml_string = value_contents
											.getNodeValue();
								}
								...
								epp_ResultValue value = new epp_ResultValue(
										...
										(good_xml
												&& value_contents
														.getFirstChild() != null ? value_contents
							...
							if (a_node.getNodeName().equals("reason")) {
								ext_value.m_reason = a_node.getFirstChild()
										.getNodeValue();
							}
						}
					...
					result.m_ext_values = (epp_ExtResultValue[]) convertListToArray(
							(new epp_ExtResultValue()).getClass(), ext_values);
		...
		generic_response.m_results = (epp_Result[]) convertListToArray(
				(new epp_Result()).getClass(), results);
		...
		// is common
		NodeList extension_nodes = ((Element) response_node)
		...
		if (extension_nodes.getLength() == 0) {
			...
			debug(DEBUG_LEVEL_TWO,
					method_name,
					"response extension node count ["
							+ extension_children.getLength() + "]");
			...
			for (int count = 0; count < extension_children.getLength(); count++) {
				try {
					...
					debug(DEBUG_LEVEL_TWO, method_name,
							"here's an extension xml string ["
									+ extension_string + "]");
		...
		NodeList trans_id_nodes = ((Element) response_node)
				.getElementsByTagName("trID");
		...
		NodeList msg_queue_nodes = ((Element) response_node)
				.getElementsByTagName("msgQ");
		...
		if (msg_queue_nodes.getLength() > 0) {
			...
			msg_queue.m_count = Integer.parseInt(msg_queue_element
					.getAttribute("count"));
			msg_queue.m_id = msg_queue_element.getAttribute("id");

			NodeList queue_date_nodes = msg_queue_element
					.getElementsByTagName("qDate");
			if (queue_date_nodes.getLength() > 0) {
				msg_queue.m_queue_date = queue_date_nodes.item(0)
						.getFirstChild().getNodeValue();
			}

			NodeList the_msg_nodes = msg_queue_element
			...
			if (the_msg_nodes.getLength() > 0) {
				...
				msg_queue.m_msg.m_value = msg_element.getFirstChild()
						.getNodeValue();
			}
	...
	/**
	 * Given the node list, it finds the node for the given name Please see the
	 * Xerces documentation for more information regarding Nodes and Nodelists
	 * 
	 * @param Nodelist
	 *            XML nodes
	 * @param String
	...
	protected Node getNode(NodeList node_list, String node_name) {
		...
		for (int count = 0; count < node_list.getLength(); count++) {
			...
			debug(DEBUG_LEVEL_TWO, method_name,
					"Node's name [" + a_node.getNodeName() + "]");
	 ...
	 * calls getNode() and casts the result into an Element. Please see the
	 * Xerces documentation for more information regarding Elements and
	 * Nodelists.
	 * 
	 * @param Nodelist
	 *            XML nodes
	...
	/**
	 * Using pre-set response XML, returns the document's Element. Please see
	 * the Xerces documentation for more information regarding Parsers,
	 ...
	 *             if the XML document does not support Traversal v2.0
	 */
	protected Element getDocumentElement() throws IOException, SAXException {
		...
		if (!document.isSupported("Traversal", "2.0"))
			throw new RuntimeException(
					"This DOM Document does not support Traversal");
	...
	/**
	 * Given a Document, this method create the root epp tag and populates its
	 * attributes with xmlns, xmlns:xsi, and xsi:schemaLocation values.
	 * 
	 * @param Document
	 *            which implements the createElement() method
	...
	protected String createXMLFromDoc(Document doc) throws IOException {
		...
		debug(DEBUG_LEVEL_THREE, method_name, "Entered");
		debug(DEBUG_LEVEL_THREE, method_name, "Entering with doc:"
				+ createXMLSnippetFromDoc(doc));
		...
		debug(DEBUG_LEVEL_THREE, method_name, "Leaving with output:"
				+ stringOut.toString());
	...
	/**
	 * Given a Document with all of its children elements appended, converts
	 * this document to an XML Snippet. The difference between this and
	 * createXMLFromDoc() is that the XML headers are not included.
	 * 
	 * @param Document
	 *            the complete XML Document
	 * @return String the XML String
	...
	protected String createXMLSnippetFromElement(Element ele)
			throws TransformerFactoryConfigurationError, TransformerException {
		String method_name = "createXMLSnippetFromElement()";
		debug(DEBUG_LEVEL_THREE, method_name, "Entered");
	 ...
	 * 
	 * @param Document
	 *            the Document instance that is creating the Elements
	 * @param Element
	 *            the Element that will contain the new Element created
	 * @param String
	 *            the tag name for the new Element
	 * @param String
	 *            the String value to put into the tag. Can be null for an empty
	 *            tag.
	 * @return Element the newly created Element which was added to the
	 *         containing Element
	 */
	protected Element addXMLElement(Document doc, Element containing_element,
			String tag_name, String value) {
	...
	/**
	 * Converts a List of any size to a Java Object array that can be cast into
	 * an array of any class. All of the objects in the List must be of the same
	 * class otherwise a run-time exception will be throws. Example:</P>
	 * 
	 * <PRE>
	 ...
	 * 
	 * @param Class
	 *            the Class of the resulting array.
	 * @param List
	 *            the List of instances of the given Class
	 * @return Object the newly created array.
	 */
	public static Object convertListToArray(Class the_class, List the_list) {
		Object[] the_array = (Object[]) Array.newInstance(the_class,
				the_list.size());
		int index = 0;
	...
	/**
	 * Converts a List of any size of Strings to a Java Object array. A
	 * convenience version of the convertListToArray() method. Example:</P>
	 * 
	 * <PRE>
	 ...
	 * 
	 * @param List
	 *            the List of instances of Strings
	 * @return String[] the resulting String array
	 */
	public static String[] convertListToStringArray(List the_list) {
		String[] the_array = (String[]) Array.newInstance(
				(new String()).getClass(), the_list.size());
		int index = 0;
		for (Iterator it = the_list.iterator(); it.hasNext();) {
	...
	/**
	 * Given a list of String values, create Elements all with the same tag name
	 * and all going under the same Element wrapper.
	 * 
	 * @param Document
	 *            the Document instance that is creating the Elements
	 * @param Element
	 *            the Element that will contain the new Elements created
	 * @param String
	 *            the tag name to use for all the new Elements
	 * @param List
	 *            the List of String values to put into the new Elements.
	 */
	protected void stringListToXML(Document doc, Element wrapper,
			String tag_name, List list) {
	 ...
	 *            the tag's name
	 * @param epp_AuthInfo
	 *            element that contains authorization information
	 * @return Element the resulting EPP <Object:authInfo> Element
	 */
	protected Element prepareAuthInfo(Document doc, String object_name,
			epp_AuthInfo auth_info) {
		Element auth_info_element = doc
				.createElement(object_name + ":authInfo");
		...
		Element authinfo_sub_el = addXMLElement(doc, auth_info_element,
				object_name + ":" + auth_info.m_type, auth_info.m_value);
	...
	/**
	 * Given the chkData XML node, it extracts the check value, object
	 * availability and reason text. Please see the Xerces documentation for
	 * more information regarding Nodes.
	 * 
	 * @param Node
	 *            EPP chkData node
	 * @return epp_CheckResult[] the generic check results array
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	 *             if the node list does not contain the data
	 */
	protected epp_CheckResult[] parseGenericCheckResults(Node chkdata_node)
		...
		if (chkdata_node == null
				|| chkdata_node.getChildNodes().getLength() == 0) {
			throw new epp_XMLException("missing check results");
		}

		NodeList check_result_list = chkdata_node.getChildNodes();
		debug(DEBUG_LEVEL_TWO, method_name, "chkData's node count ["
				+ check_result_list.getLength() + "]");
		...
		for (int count = 0; count < check_result_list.getLength(); count++) {
			...
			if (!a_node.getNodeName().endsWith(":cd"))
				continue;
			...
			for (int i = 0; i < one_check_list.getLength(); i++) {
				...
				if (the_node.getNodeName().endsWith(":name")
					...
					chk_result.m_value = the_node.getFirstChild()
							.getNodeValue();
					chk_result.m_avail = ((Element) the_node).getAttribute(
							"avail").equals("1") ? true : false;
				}
				if (the_node.getNodeName().endsWith(":reason")) {
					chk_result.m_reason = the_node.getFirstChild()
							.getNodeValue();
					chk_result.m_lang = ((Element) the_node)
							.getAttribute("lang");
				}
			}
		...
		debug(DEBUG_LEVEL_THREE, method_name, "Leaving");
		return (epp_CheckResult[]) convertListToArray(epp_CheckResult.class,
				check_list);
	}

	/**
	 * Given an array of epp_CheckResult's, finds the matching value and returns
	 * it's exists value in a Boolean object. This method works for Check
	 * Results on any object.
	 * 
	 * @param epp_CheckResult
	 *            [] array of check results.
	 * @param String
	 *            the value for which to search
	 * @return Boolean the exists boolean in a Boolean object. Will return null
	 *         if the array is null or if the value was not found.
	 * @deprecated Please use getAvailResultFor(epp_CheckResult[]
	 *             check_results,String check_value)
	 */
	public static Boolean getCheckResultFor(epp_CheckResult[] check_results,
		...
		if (wanted_result != null)
			wanted_result = new Boolean(!wanted_result.booleanValue());
	...
	/**
	 * Given an array of epp_CheckResult's, finds the matching value and returns
	 * it's avail value in a Boolean object. This method works for Check Results
	 * on any object.
	 * 
	 * @param epp_CheckResult
	 *            [] array of check results.
	...
	protected Node getExtensionNode(String extension_string)
		...
		parser.parse(new InputSource(new ByteArrayInputStream(extension_string
				.getBytes())));
		Document document = parser.getDocument();
	 ...
	 */
	protected Element prepareExtensionElement(Document doc, Element command,
		...
		if (extensions != null) {
			...
			for (int count = 0; count < extensions.length; count++) {
				...
				Element extension_subElement = extension.toXMLElement(doc);
				extension_element.appendChild(extension_subElement);
			}
			debug(DEBUG_LEVEL_THREE, method_name, "Leaving with element");
			return extension_element;
		}
		debug(DEBUG_LEVEL_THREE, method_name, "Leaving with null");
		return null;
	}
	 ...
	 */
	public static String transferStatusToString(
			epp_TransferStatusType status_type) {
	...
	public static String getPanData(NodeList pan_data_list,
			String namespace_uri, epp_PanData pan_data) throws epp_XMLException {
		String the_key = "";
		...
		for (int count = 0; count < pan_data_list.getLength(); count++) {
			...
			if (a_node.getLocalName().equals("id")
				...
				pan_data.setResult((paResult.equals("1")
						|| paResult.equals("true") ? true : false));
				the_key = a_node.getFirstChild().getNodeValue();
			}
			if (a_node.getLocalName().equals("paDate")) {
				pan_data.setDate(a_node.getFirstChild().getNodeValue());
			}
	...
	public static epp_TransID getTransID(Node trans_id_node) {
		...
		for (int count = 0; count < inner_nodes.getLength(); count++) {
			...
			if (a_node.getLocalName().equals("clTRID")) {
				trans_id.m_client_trid = a_node.getFirstChild().getNodeValue();
			}
			if (a_node.getLocalName().equals("svTRID")) {
