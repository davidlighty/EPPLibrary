...
public class EPPContactInfo extends EPPContactBase implements epp_ContactInfo
{
        ...
        try
        {
            ...
            for (int count = 0; count < contact_info_result_list.getLength(); count++)
            {
                ...
                if ( a_node.getNodeName().equals("contact:voice") )
                {
                    ...
                    action_response_.m_voice.m_value = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue());
                ...
                if ( a_node.getNodeName().equals("contact:fax") )
                {
                    ...
                    action_response_.m_fax.m_value = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue());
                    action_response_.m_fax.m_extension = ((Element)a_node).getAttribute("x");
                }
                if ( a_node.getNodeName().equals("contact:email") ) { action_response_.m_email = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
                ...
                if ( a_node.getNodeName().equals("contact:clID") ) { action_response_.m_client_id = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }

                if ( a_node.getNodeName().equals("contact:crID") ) { action_response_.m_created_by = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
                if ( a_node.getNodeName().equals("contact:crDate") ) { action_response_.m_created_date = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
                if ( a_node.getNodeName().equals("contact:upID") ) { action_response_.m_updated_by = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
                if ( a_node.getNodeName().equals("contact:upDate") ) { action_response_.m_updated_date = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }

                if ( a_node.getNodeName().equals("contact:trDate") ) { action_response_.m_transfer_date = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
    ...
    private epp_ContactNameAddress addressFromXML(Node address_node) throws epp_XMLException
    {
        ...
        for (int count = 0; count < address_nodes.getLength(); count++)
        {
            ...
            if ( a_node.getNodeName().equals("contact:name") ) { name_address.m_name = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
            if ( a_node.getNodeName().equals("contact:org") ) { name_address.m_org = (a_node.getFirstChild()==null?"":a_node.getFirstChild().getNodeValue()); }
            ...
            if ( a_node.getNodeName().equals("contact:addr") )
            {
                ...
                for (int count2 = 0; count2 < address_sub_nodes.getLength(); count2++)
                {
                    ...
                    if ( an_inner_node.getNodeName().equals("contact:street") )
                    {
                        ...
                        {
                            address.m_street1 = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue());
                        }
                        else if ( address.m_street2 == null )
                        {
                            address.m_street2 = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue());
                        }
                        else if ( address.m_street3 == null )
                        {
                            address.m_street3 = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue());
                        }
                        // No "else"... just ignoring situation where the
                        // server sends more street values than we expect.
                    }

                    if ( an_inner_node.getNodeName().equals("contact:city") ) { address.m_city = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue()); }
                    if ( an_inner_node.getNodeName().equals("contact:sp") ) { address.m_state_province = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue()); }
                    if ( an_inner_node.getNodeName().equals("contact:pc") ) { address.m_postal_code = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue()); }
                    if ( an_inner_node.getNodeName().equals("contact:cc") ) { address.m_country_code = (an_inner_node.getFirstChild()==null?"":an_inner_node.getFirstChild().getNodeValue()); }
                }
