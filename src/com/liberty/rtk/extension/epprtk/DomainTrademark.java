...
/**
 * This class is used to exchange Domain Trademark data with the Liberty
 * .info/.in/.mobi Registry. There is not name space to specify which registry
 * it was used for since in the begining there was only .info. Later on as .in
 * and .mobi introduced and different registry has different settings for
 * trademark infomation, it has to be optional for some fields in trademark to
 * adapt to different registry. Here is the list of required fields:
 * 
 * INFO <name/> <country/> <number/> <date/> IN <name/> <country/>
 * <ownerCountry/> <number/> <date/> MOBI <name/> <country/> <number/>
 * <appDate/> <regDate/>
 * 
 * registrars are free to fill infomation in trademark and it is registry's
 * responsibility to validate those infomation.
 * 
 * The data should only be used in the OT&E environment and during the live
 * Sunrise period of the registry's operation. During this time the Registry
 * will required the trademark data on domain creation.
 * 
...
public class DomainTrademark extends EPPXMLBase implements epp_Extension {
	...
	/**
	 * Constructor with Domain Trademark Unspec XML string to automatically
	 * parse.
	 * 
	 * @param xml
	...
	/**
	 * Accessor method for the trademark country data member. Must be a
	...
	/**
	 * Accessor method for the trademark owner's country data member. Must be a
	 * two-character ISO country code.
	 * 
	 * @param value
	 *            String
	...
	/**
	 * Accessor method for the trademark date data member. Returns the date as a
	 * Date object. If the date string cannot be parsed into a date object, then
	 * null is returned.
	 * 
	...
	public Date getDate() {
		Date the_date;
		try {
			the_date = DATE_FMT.parse(tm_date_);
	...
	/**
	 * Converts the trademark data into XML to be put into the extension section
	 * of the request. Implemented method from org.openrtk.idl.epprtk.epp_Unspec
	 * interface.
	 * 
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	...
	public String toXML() throws epp_XMLException {
		...
		Element trademark = toXMLElement(doc);
	...
	/**
	 * Parses an XML String of trademark data from the extension section of a
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			...
			/*
			 * EPPRTK has a bug which assumes there can be mutiple
			 * <extension></extension> elements in an command or response, so
			 * the xml snippet becomes <extension> <trademark><
			 * ...></trademark></extension>.
			 */
			...
			NodeList trademark_node_list = extension_node
					.getElementsByTagName("trademark");
			...
			debug(DEBUG_LEVEL_TWO,
					method_name,
					"trademark_node_list's node count ["
							+ trademark_node_list.getLength() + "]");
			...
			for (int count = 0; count < detail_node_list.getLength(); count++) {
				...
				// if ( a_node.getNodeName().equals("date") ) { try { tm_date_ =
				// DATE_FMT.parse(a_node.getFirstChild().getNodeValue()); }
				// catch(ParseException xcp) { throw new
				// epp_XMLException("bad trademark date format"); } }
				if (a_node.getNodeName().equals("date")) {
					tm_date_ = a_node.getFirstChild().getNodeValue();
				}
				if (a_node.getNodeName().equals("appDate")) {
					tm_app_date_ = a_node.getFirstChild().getNodeValue();
				}
				if (a_node.getNodeName().equals("regDate")) {
					tm_reg_date_ = a_node.getFirstChild().getNodeValue();
				}
				if (a_node.getNodeName().equals("country")) {
					tm_country_ = a_node.getFirstChild().getNodeValue();
				}
				if (a_node.getNodeName().equals("ownerCountry")) {
					tm_owner_country_ = a_node.getFirstChild().getNodeValue();
				}
				if (a_node.getNodeName().equals("number")) {
