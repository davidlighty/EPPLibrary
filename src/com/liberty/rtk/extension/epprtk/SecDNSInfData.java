...
public class SecDNSInfData extends EPPXMLBase implements epp_Extension {
	...
	public void setMaxSigLife(int value) {
		ds_max_sig_life_ = value;
	}

	public int getMaxSigLife() {
		return ds_max_sig_life_;
	}
	...
	/**
	 * This would parse a response from the server with the <secDNS:infData>
	 * extension. Implemented method from org.openrtk.idl.epprtk.epp_Extension
	 * interface.
	 * 
	 * @param A
		...
		try {
			...
			for (int count = 0; count < infData_node_list.getLength(); count++) {
				...
				if (a_node.getNodeName().equals("secDNS:maxSigLife")) {
					ds_max_sig_life_ = Integer.parseInt(a_node.getFirstChild()
							.getNodeValue());
				}
				if (a_node.getNodeName().equals("secDNS:dsData")) {
					...
					for (int i = 0; i < a_subNode.getLength(); i++) {
						...
						if (the_node.getNodeName().equals("secDNS:keyTag")) {
							temp_ds_data.setKeyTag(Integer.parseInt(the_node
									.getFirstChild().getNodeValue()));
						}
						if (the_node.getNodeName().equals("secDNS:alg")) {
							temp_ds_data.setAlg(Integer.parseInt(the_node
									.getFirstChild().getNodeValue()));
						}
						if (the_node.getNodeName().equals("secDNS:digestType")) {
							temp_ds_data.setDigestType(Integer
									.parseInt(the_node.getFirstChild()
											.getNodeValue()));
						}
						if (the_node.getNodeName().equals("secDNS:digest")) {
							temp_ds_data.setDigest(the_node.getFirstChild()
									.getNodeValue());
						}
						// if (
						// the_node.getNodeName().equals("secDNS:maxSigLife")) {
						...
						if (the_node.getNodeName().equals("secDNS:keyData")) {
							...
							for (int j = 0; j < a_subNode_2.getLength(); j++) {
								...
								if (the_node_2.getNodeName().equals(
										"secDNS:flags")) {
									key_data.setFlags(Integer
											.parseInt(the_node_2
													.getFirstChild()
													.getNodeValue()));
								}
								...
								if (the_node_2.getNodeName().equals(
										"secDNS:alg")) {
									key_data.setAlg(Integer.parseInt(the_node_2
											.getFirstChild().getNodeValue()));
								}
								if (the_node_2.getNodeName().equals(
										"secDNS:pubKey")) {
									key_data.setPubKey(the_node_2
											.getFirstChild().getNodeValue());
								}
							}
