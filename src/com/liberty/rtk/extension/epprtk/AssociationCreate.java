...
public class AssociationCreate extends EPPXMLBase implements epp_Extension {
	...
	public void setContacts(Collection value) {
		contacts_ = value;
	}
	...
	public String toXML() throws epp_XMLException {
		...
		if (contacts_.size() == 0)
			throw new epp_XMLException("contacts missing");
		...
		for (Iterator it = contacts_.iterator(); it.hasNext();)
			e.appendChild(((AssociationContact) it.next()).getElement(doc));
		...
		try {
			...
			throw new epp_XMLException(
					"IOException in building membership:create XML ["
							+ xcp.getMessage() + "]");
		}

		debug(DEBUG_LEVEL_THREE, method_name, "The extension XML is: ["
				+ membership_create_xml + "]");
	...
	/**
	 * This would parse a response from the server with the <membership:create>
	 * extension. but <membership:create> is only used in the <domain:create>
	 * request.<br>
