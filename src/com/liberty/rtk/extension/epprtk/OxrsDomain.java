...
public class OxrsDomain extends EPPXMLBase implements epp_Extension {
	 ...
	 * This qualifies the maintainerUrl, domainRoid in domain:create request and
	...
	public String toXML() throws epp_XMLException {
		...
		debug(DEBUG_LEVEL_THREE, method_name,
				"The OxrsDomain extension XML is: [" + oxrs_xml + "]");
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			...
			debug(DEBUG_LEVEL_TWO, method_name, "oxrs_node_list's node count ["
					+ oxrs_node_list.getLength() + "]");
	...
	protected void setAttribute(Element oxrs) {
		...
		oxrs.setAttribute("xmlns:xsi",
				"http://www.w3.org/2001/XMLSchema-instance");
		oxrs.setAttribute("xsi:schemaLocation",
				"urn:afilias:params:xml:ns:oxrs-1.1 oxrs-1.1.xsd");
	}
			...
			if (chg_) {
				...
				if (maintainerURL_ != null && maintainerURL_.length() != 0) {
					ExtUtils.addXMLElement(doc, chg_element,
							"oxrs:maintainerUrl", maintainerURL_);
				}
			}
		} else if (command_.equals("create")) {
			if (maintainerURL_ != null && maintainerURL_.length() != 0) {
				ExtUtils.addXMLElement(doc, oxrs, "oxrs:maintainerUrl",
						maintainerURL_);
			}
		}
