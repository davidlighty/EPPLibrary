...
/**
 * This extension is the .org domain's protocol in which it was registered: RRP
 * or EPP. The .org EPP server will optionally return the extension XML for this
 * class in the &lt;domain:info&gt; response. If the extension is not present
 * and this handler is called, no exception will be thrown and the protocol will
 * be set to "epp". The .org registry specifies that the default protocol value
 * is "epp".
 * 
...
public class DomainProtocol extends EPPXMLBase implements epp_Extension {
	 ...
	 * <domain:info>) to automatically parse.
	 * 
	 * @param xml
	 ...
	 * 
	 * @param value
	 *            The new value for the domain protocol
	 */
	public void setProtocol(String value) {
		domain_protocol_ = value;
	}
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			...
			NodeList infData_node_list = infData_node
					.getElementsByTagName("oxrs:infData");
			...
			debug(DEBUG_LEVEL_TWO,
					method_name,
					"protocol_node_list's node count ["
							+ infData_node_list.getLength() + "]");

			for (int count = 0; count < infData_node_list.getLength(); count++) {
				Node a_node = infData_node_list.item(count);

				// The protocol is contained inside the oxrs:infData, so let's
				// dig inside there
				if (a_node.getNodeName().equals("oxrs:infData")) {
					NodeList protocol_node_list = ((Element) a_node)
							.getElementsByTagName("oxrs:protocol");
					if (protocol_node_list.getLength() == 0) {
						// no protocol child elements
						throw new epp_XMLException(
								"unparsable or missing domain protocol (oxrs:protocol)");
					}
