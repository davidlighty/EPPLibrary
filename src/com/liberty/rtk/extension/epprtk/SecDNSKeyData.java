...
public class SecDNSKeyData {
	...
	public Element getElement(Document doc) throws epp_XMLException {
		...
		if (ds_flags_ < 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:flags invalid ("
					+ ds_flags_ + ")");
		if (ds_protocol_ < 0)
			throw new epp_XMLException(
					"secDNS:dsData.secDNS:protocol invalid (" + ds_protocol_
							+ ")");
		if (ds_alg_ < 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:alg invalid ("
					+ ds_alg_ + ")");
		if (ds_pub_key_ == null || ds_pub_key_.length() == 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:pub_key invalid ("
					+ ds_pub_key_ + ")");

		ExtUtils.addXMLElement(doc, e, "secDNS:flags",
				String.valueOf(ds_flags_));
		ExtUtils.addXMLElement(doc, e, "secDNS:protocol",
				String.valueOf(ds_protocol_));
	...
	public String toString() {
		return "[flags: " + ds_flags_ + "|protocol: " + ds_protocol_ + "|alg: "
				+ ds_alg_ + "|pubKey: " + ds_pub_key_ + "]";
	}
}
