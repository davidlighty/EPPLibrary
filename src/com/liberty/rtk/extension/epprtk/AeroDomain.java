...
public class AeroDomain extends AeroBase {
	 ...
	 * This qualifies the ensAuthID name of the domain(s) in domain:create
	 * request and domain:info response. Implemented method from
	...
	public String toXML() throws epp_XMLException {
		...
		debug(DEBUG_LEVEL_THREE, method_name,
				"The AeroDomain extension XML is: [" + aero_xml + "]");
	...
	private void addXMLElement(Document doc, Element element, EnsInfo ens_info) {
		...
		if (ens_info.getEnsClass() != null) {
			for (Iterator itor = ens_info.getEnsClass().iterator(); itor
					.hasNext();) {
				ExtUtils.addXMLElement(doc, element, "aero:ensClass",
						(String) itor.next());
			}
		}

		if (ens_info.getRegistrantGroup() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:registrantGroup",
					ens_info.getRegistrantGroup());
		}

		if (ens_info.getEnsO() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:ensO",
					ens_info.getEnsO());
		}

		if (ens_info.getRequestType() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:requestType",
					ens_info.getRequestType());
		}

		if (ens_info.getRegistrationType() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:registrationType",
					ens_info.getRegistrationType());
		}

		if (ens_info.getCredentialsType() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:credentialsType",
					ens_info.getCredentialsType());
		}

		if (ens_info.getCredentialsValue() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:credentialsValue",
					ens_info.getCredentialsValue());
		}

		if (ens_info.getCodeValue() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:codeValue",
					ens_info.getCodeValue());
		}

		if (ens_info.getUniqueIdentifier() != null) {
			ExtUtils.addXMLElement(doc, element, "aero:uniqueIdentifier",
					ens_info.getUniqueIdentifier());
		}
	}
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			...
			debug(DEBUG_LEVEL_TWO, method_name, "aero_node_list's node count ["
					+ aero_node_list.getLength() + "]");
	...
	public Element toXMLElement(Document doc) throws epp_XMLException {
		...
		if (ensAuthID_ != null) {
			if (change != null) {
				ExtUtils.addXMLElement(doc, change, "aero:ensAuthID",
						ensAuthID_);
			} else {
		...
		if (ensAuthKey_ != null) {
			if (change != null) {
				ExtUtils.addXMLElement(doc, change, "aero:ensAuthKey",
						ensAuthKey_);
			} else {
				ExtUtils.addXMLElement(doc, aero, "aero:ensAuthKey",
						ensAuthKey_);
			}
		}
