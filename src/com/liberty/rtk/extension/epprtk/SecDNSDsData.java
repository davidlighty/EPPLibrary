...
public class SecDNSDsData {
	...
	public void setDigestType(int value) {
		ds_digest_type_ = value;
	}

	public int getDigestType() {
		return ds_digest_type_;
	}
	...
	public void setKeyData(SecDNSKeyData value) {
		ds_key_data_ = value;
	}

	public SecDNSKeyData getKeyData() {
		return ds_key_data_;
	}
	...
	public Element getElement(Document doc) throws epp_XMLException {
		...
		if (ds_key_tag_ < 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:keyTag invalid ("
					+ ds_key_tag_ + ")");
		if (ds_alg_ < 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:alg invalid ("
					+ ds_alg_ + ")");
		if (ds_digest_type_ < 0)
			throw new epp_XMLException(
					"secDNS:dsData.secDNS:digestType invalid ("
							+ ds_digest_type_ + ")");
		if (ds_digest_ == null || ds_digest_.length() == 0)
			throw new epp_XMLException("secDNS:dsData.secDNS:digest invalid ("
					+ ds_digest_ + ")");

		ExtUtils.addXMLElement(doc, e, "secDNS:keyTag",
				String.valueOf(ds_key_tag_));
		ExtUtils.addXMLElement(doc, e, "secDNS:alg", String.valueOf(ds_alg_));
		ExtUtils.addXMLElement(doc, e, "secDNS:digestType",
				String.valueOf(ds_digest_type_));
		ExtUtils.addXMLElement(doc, e, "secDNS:digest", ds_digest_);

		if (ds_key_data_ != null)
			e.appendChild(ds_key_data_.getElement(doc));

		return e;
	}

	public String toString() {
		return "[keyTag: " + ds_key_tag_ + "|alg: " + ds_alg_ + "|digestType: "
				+ ds_digest_type_ + "|digest: " + ds_digest_ + "|keyData: "
				+ ds_key_data_ + "]";
	}
}
