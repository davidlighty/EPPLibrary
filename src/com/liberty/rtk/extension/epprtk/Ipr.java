...
public class Ipr extends EPPXMLBase implements epp_Extension {
	...
	/**
	 * Converts the ipr data into XML to be put into the extension section of
	 * the request. Implemented method from org.openrtk.idl.epprtk.epp_Unspec
	 * interface.
	 * 
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	...
	/**
	 * Parses an XML String of ipr data from the extension section of a response
	 * from the Registry. Implemented method from
	 * org.openrtk.idl.epprtk.epp_Unspec interface.
	 * 
	 * @param A
	 *            new ipr Unspec XML String to parse
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	 *             if the response XML is not parsable or does not contain the
	 *             expected data
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			 ...
			 * the xml snippet becomes <extension> <ipr><
			 * ...></ipr></extension>.
			 */
			...
			for (int count = 0; count < detail_node_list.getLength(); count++) {
				...
				if (a_node.getNodeName().equals(prefix + "entitlement")) {
					tm_data_.setEntitlement(a_node.getFirstChild()
							.getNodeValue());
				}
				...
				if (a_node.getNodeName().equals(prefix + "preVerified")) {
					tm_data_.setPreVerified(a_node.getFirstChild()
							.getNodeValue());
				}
			}
	...
	private static void setNamespace(Element element, String tld)
		...
		if (tld.equals("asia")) {
			...
			element.setAttribute("xmlns:ipr",
					"urn:afilias:params:xml:ns:ipr-1.0");
			element.setAttribute("xsi:schemaLocation",
					"urn:afilias:params:xml:ns:ipr-1.0 ipr-1.0.xsd");
		} else {
			element.setAttribute("xmlns:xsi",
					"http://www.w3.org/2001/XMLSchema-instance");
			element.setAttribute("xmlns:ipr",
			...
			element.setAttribute("xsi:schemaLocation",
					"urn:afilias:params:xml:ns:ipr-1.1 ipr-1.1.xsd");
		}
	}
	...
	private void addIPRData(Document doc, Element node, IprData data)
			throws epp_XMLException {
		if (tm_data_.getName() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "name", data.getName());
		if (tm_data_.getCountry() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "ccLocality",
					data.getCountry());
		if (tm_data_.getNumber() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "number",
					data.getNumber());
		if (tm_data_.getRegDate() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "regDate",
					data.getRegDate());
		if (tm_data_.getAppDate() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "appDate",
					data.getAppDate());
		if (tm_data_.getIprClass() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "class",
					data.getIprClass());
		if (tm_data_.getEntitlement() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "entitlement",
					data.getEntitlement());
		...
		if (tm_data_.getPreVerified() != null)
			ExtUtils.addXMLElement(doc, node, prefix + "preVerified",
					data.getPreVerified());
	}
