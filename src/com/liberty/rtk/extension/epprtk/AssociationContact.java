...
public class AssociationContact {
	...
	public Element getElement(Document doc) throws epp_XMLException {
		...
		if (contactValue_ == null || contactValue_.length() == 0)
			throw new epp_XMLException("association:contact:id invalid ("
					+ contactValue_ + ")");

		if (type_ == null || type_.length() == 0)
			throw new epp_XMLException("association:contact:type invalid ("
					+ type_ + ")");
		...
		ExtUtils.addXMLElement(doc, auth_info_element, "association:pw",
				authInfo_);
