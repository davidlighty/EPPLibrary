...
public class AssociationUpdate extends EPPXMLBase implements epp_Extension {
	...
	public void setAdd(AssociationAdd value) {
		add_ = value;
	}

	public AssociationAdd getAdd() {
		return add_;
	}

	public void setRem(AssociationRem value) {
		rem_ = value;
	}

	public AssociationRem getRem() {
		return rem_;
	}
	...
	public String toXML() throws epp_XMLException {
		...
		e.setAttribute("xsi:schemaLocation",
				"urn:afilias:params:xml:ns:association-1.0 association-1.0.xsd");

		if (add_ != null && !add_.isEmpty())
			e.appendChild(add_.getElement(doc));
		if (rem_ != null && !rem_.isEmpty())
			e.appendChild(rem_.getElement(doc));
		...
		try {
			...
			throw new epp_XMLException(
					"IOException in building association:update XML ["
							+ xcp.getMessage() + "]");
		}

		debug(DEBUG_LEVEL_THREE, method_name, "The extension XML is: ["
				+ membership_update_xml + "]");
	...
	/**
	 * This would parse a response from the server with the <association:update>
	 * extension. but <association:update> is only used in the <domain:update>
	 * request.<br>
