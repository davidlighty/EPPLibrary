...
public class IDN extends EPPXMLBase implements epp_Extension {
	...
	/**
	 * Do not initialize isOtne to be true unless you are doing otne and need to
	 * perform sending invalid IDN request
	 * 
	...
	/**
	 * Accessor method for the EPP command for this IDN extension. Typical
	 * values are "create" and "check", though this is not validated.
	 * 
	 * @param value
	...
	/**
	 * Accessor method for the IDN ISO locale/script. Format is governed by
	 * RFC3066 (eg. "en-CA", "de", etc...)
	 * 
	 * @param value
	 *            The new IDN script.
	 */
	 ...
	 * This qualifies the punycode name of the domain(s) in domain:create and
	 * domain:check commands. Implemented method from
	...
	public String toXML() throws epp_XMLException {
		if (!isOtne && (script_ == null || script_.equals(""))) {
			throw new epp_XMLException("missing locale/script for idn:"
					+ command_ + " extension ");
		}
		...
		debug(DEBUG_LEVEL_THREE, method_name, "The IDN extension XML is: ["
				+ idn_xml + "]");
	...
	/**
	 * This method is used to parse the IDN extension contained in an EPP
	 * response.<br>
	 * 
	 * @param A
	 *            new IDN response extension XML String to parse
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	...
	public void fromXML(String xml) throws epp_XMLException {
		...
		try {
			...
			debug(DEBUG_LEVEL_TWO, method_name, "idn_node_list's node count ["
					+ idn_node_list.getLength() + "]");
	...
	public Element toXMLElement(Document doc) throws epp_XMLException {
		...
		idn.setAttribute("xmlns:xsi",
				"http://www.w3.org/2001/XMLSchema-instance");
		idn.setAttribute("xsi:schemaLocation",
				"urn:ietf:params:xml:ns:idn-1.0 idn-1.0.xsd");
		...
		if (command_.equals("update")) {
			if (getChg()) {
				...
				ExtUtils.addXMLElement(doc, change_element, "idn:script",
						script_);
