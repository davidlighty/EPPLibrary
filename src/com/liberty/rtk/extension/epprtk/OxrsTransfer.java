...
/**
 * When transfering a domain from an RRP-based registrar to an EPP-based on, the
 * .org registry requires new EPP contact data to be supplied which replaces the
 * placeholder contact data when the domain is transferred.
 * 
...
public class OxrsTransfer extends EPPXMLBase implements epp_Extension {
	 ...
	 * 
	 * @param value
	 *            The new single registrant contact.
	 */
	public void setRegistrant(String value) {
		registrant_contact_ = value;
	}
	...
	public String getRegistrant() {
		return registrant_contact_;
	}

	/**
	 * Accessor method for the array of contacts to be associated with the
	 * domain object
	 * 
	 * @param value
	...
	public epp_DomainContact[] getContacts() {
		return other_contacts_;
	}

	/**
	 * Converts the test number data into XML to be put into the unspec section
	 * of the request. Implemented method from org.openrrc.rtk.epprtk.epp_Unspec
	 * interface.
	 * 
	...
	public String toXML() throws epp_XMLException {
		...
		if (registrant_contact_ == null) {
			throw new epp_XMLException(
					"missing registrant contact in oxrs:transfer");
		}
		if (other_contacts_ == null || other_contacts_.length == 0) {
			throw new epp_XMLException(
					"missing other contacts in oxrs:transfer");
		}

		Document doc = new DocumentImpl();

		Element oxrs_transfer = toXMLElement(doc);
		...
		debug(DEBUG_LEVEL_THREE, method_name,
				"The transfer extension XML is: [" + oxrs_transfer_xml + "]");
	...
	/**
	 * This would parse a response from the server with the <oxrs:transfer>
	 * extension. but <oxrs:transfer> is only used in the <domain:transfer>
	 * request.<br>
	 * Implemented method from org.openrtk.idl.epprtk.epp_Extension interface.
	 * 
	 * @param A
	 *            new oxrs transfer Extension XML String to parse
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	...
	public Element toXMLElement(Document doc) throws epp_XMLException {
		...
		oxrs_transfer.setAttribute("xmlns:oxrs",
				"urn:ietf:params:xml:ns:oxrs-1.0");
		oxrs_transfer.setAttribute("xmlns:xsi",
				"http://www.w3.org/2001/XMLSchema-instance");
		oxrs_transfer.setAttribute("xsi:schemaLocation",
				"urn:ietf:params:xml:ns:oxrs-1.0 oxrs-1.0.xsd");

		addXMLElement(doc, oxrs_transfer, "oxrs:registrant",
				registrant_contact_);
		...
		if (other_contacts_ != null && other_contacts_.length > 0) {
			...
			for (Iterator it = contact_list.iterator(); it.hasNext();) {
				epp_DomainContact domain_contact = (epp_DomainContact) it
						.next();
				Element contact_element = addXMLElement(doc, oxrs_transfer,
						"oxrs:contact", domain_contact.m_id);
				contact_element.setAttribute("type",
						domain_contact.m_type.toString());
			}
		}

		return oxrs_transfer;
