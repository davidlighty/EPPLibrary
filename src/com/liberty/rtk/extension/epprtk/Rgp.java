...
public class Rgp extends EPPXMLBase implements epp_Extension {
	...
	public void setRgpStatus(String value) {
		if (rgp_statuses_ == null)
			rgp_statuses_ = (List) new ArrayList();
		rgp_statuses_.add(value);
	}

	public List getRgpStatus() {
		return rgp_statuses_;
	}

	/**
	 * Converts the rgp data into XML to be put into the extension section of
	...
	/**
	 * Parses an XML String of rgp data from the extension section of a response
	 * from the Registry. Implemented method from
	 * org.openrtk.idl.epprtk.epp_Unspec interface.
	 * 
	 * @param A
	 *            new rgp Unspec XML String to parse
	 * @throws org.openrtk.idl.epprtk.epp_XMLException
	 *             if the response XML is not parsable or does not contain the
	 *             expected data
	...
	public Element toXMLElement(Document doc) throws epp_XMLException {
		...
		if (report_data_ != null) {
			...
			if (report_data_.getPreData() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "preData",
						report_data_.getPreData());
			if (report_data_.getPostData() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "postData",
						report_data_.getPostData());
			if (report_data_.getDelTime() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "delTime",
						report_data_.getDelTime());
			if (report_data_.getResTime() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "resTime",
						report_data_.getResTime());
			if (report_data_.getResReason() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "resReason",
						report_data_.getResReason());
			if (report_data_.getStatement1() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "statement",
						report_data_.getStatement1());
			if (report_data_.getStatement2() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "statement",
						report_data_.getStatement2());
			if (report_data_.getOther() != null)
				ExtUtils.addXMLElement(doc, report, prefix + "other",
						report_data_.getOther());
