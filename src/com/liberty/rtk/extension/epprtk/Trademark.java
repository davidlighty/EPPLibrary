 ...
 * .info/.mobi/.in Registry. There is not name space to specify which registry
 * it was used for since in the begining there was only .info. Later on .mobi
 * introduced and different registry has different settings for trademark
 * infomation, it has to be optional for some fields in trademark to adapt to
 * different registry. Here is the list of required fields:
 * 
 * INFO <name/> <country/> <number/> <date/> MOBI <name/> <country/> <number/>
 * <appDate/> <regDate/> IN <name/> <number/> <country/> <date/> <ownerCountry/>
 * 
...
public class Trademark extends EPPXMLBase implements epp_Extension {
		...
		try {
			...
			for (int count = 0; count < detail_node_list.getLength(); count++) {
				...
				if (a_node.getFirstChild() != null)
					value = a_node.getFirstChild().getNodeValue();
	...
	private static void setNamespace(Element element, String tld)
	...
	public Element toXMLElement(Document doc) throws epp_XMLException {
		...
		if (tm_data_.isRemove()) {
			...
			if (tm_data_.getName() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "name",
						tm_data_.getName());
			if (tm_data_.getCountry() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "country",
						tm_data_.getCountry());
			if (tm_data_.getNumber() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "number",
						tm_data_.getNumber());
			if (tm_data_.getDate() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "date",
						tm_data_.getDate());
			if (tm_data_.getRegDate() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "regDate",
						tm_data_.getRegDate());
			if (tm_data_.getAppDate() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "appDate",
						tm_data_.getAppDate());
			if (tm_data_.getOwnerCountry() != null)
				ExtUtils.addXMLElement(doc, trademark, prefix + "ownerCountry",
						tm_data_.getOwnerCountry());
		}
